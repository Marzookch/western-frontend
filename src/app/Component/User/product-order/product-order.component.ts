...
export class ProductOrderComponent implements OnInit {
  ...
  onContinue() {
    if (!this.selectedShippingMethod || !this.selectedPaymentMethod) {
      ...
      this.showError = true;
      Swal.fire('Error', 'Please select both a shipping method and a payment method.', 'error');
    } else {
    ...
    QRCode.toDataURL(upiString, (err, qrCodeUrl) => {
      ...
      const interval = setInterval(() => {
        ...
        Swal.update({
          html: `
            <div>
              <p>Time left: <strong>${minutes}:${seconds < 10 ? '0' : ''}${seconds}</strong></p>
        ...
        if (timeLeft <= 0) {
          ...
          Swal.fire('QR Code Expired', 'The QR code has expired. Please try again.', 'error');
        }
  ...
  return Swal.fire({
    ...
    preConfirm: (transactionId) => {
      if (!transactionId || transactionId.length !== 6) {
        Swal.showValidationMessage('Please enter a valid 6-digit transaction ID');
        return false;
      }
